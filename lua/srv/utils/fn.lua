...
Functions.typewriter_toggle = function()
	if typewriter_state then
		...
		vim.opt_local.scrolloff = math.floor(vim.api.nvim_win_get_height(vim.api.nvim_get_current_win()) / 2)
	end
	typewriter_state = not typewriter_state -- toggle the state
	require("notify")((typewriter_state and "En" or "Dis") .. "abled typewriter scrolling", "info")
end
...
Functions.lsp.async_formatting = function(client, bufnr)
	vim.lsp.buf_request(bufnr, "textDocument/formatting", vim.lsp.util.make_formatting_params({}), function(err, res, _)
		if err then
...
Functions.snippets.snip = function(trig, name, desc, snippetType, wordTrig, trigEngine, priority)
	return {
		trig = trig,
		name = name,
		desc = desc,
